...
const PostComponents: React.FC<PostCommentProps> = ({ likes, comments, postId, userId, contact_number }) =>
{
    ...
    useEffect(() =>
    {
        ...
        setLikeCount(typeof likes === "number" ? likes : parseInt(likes || "0"));
            ...
            <div className="flex space-x-6">
                <span className="inline-flex items-center text-sm">
                    <button
                        onClick={handleLike}
                        disabled={loading}
                        className={`cursor-pointer inline-flex space-x-2 ${liked ? 'text-blue-500' : ''}`}
                    >
                        <svg className="h-5 w-5 hover:text-blue-500" x-description="Heroicon name: mini/hand-thumb-up" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                        ...
                        <span className="font-medium">{isNaN(likeCount) ? 0 : likeCount}</span>
                        <span className="sr-only">likes</span>
                    </button>
                </span>
                <span className="inline-flex items-center text-sm">
                    <button type="button" className="inline-flex space-x-2 hover:text-gray-500">
...
export default memo(PostComponents)
